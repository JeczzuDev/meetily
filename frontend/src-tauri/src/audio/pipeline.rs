...
impl ProfessionalAudioMixer {
    ...
    fn mix_window(&mut self, mic_window: &[f32], sys_window: &[f32]) -> Vec<f32> {
        ...
        // Read user-configurable gains (lock-free atomics for hot-path performance)
        let (mic_gain, sys_gain) = crate::get_audio_gains_internal();

        for i in 0..max_len {
            let mic = mic_window.get(i).copied().unwrap_or(0.0);
            let sys = sys_window.get(i).copied().unwrap_or(0.0);

            // Apply user-configurable gains
            let sum = mic * mic_gain + sys * sys_gain;

            // Soft clipping: scale down proportionally when sum exceeds ±1.0
            let sum_abs = sum.abs();
